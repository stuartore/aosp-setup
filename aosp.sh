...
' /etc/pacman.conf
	...
	sudo pacman -Syy --noconfirm --needed git git-lfs pcre2 rsync zip base-devel perl libxcrypt-compat multilib-devel fontconfig ttf-droid yay ccache make yay patch pkg-config maven gradle python-pyelftools
...
handle_sync(){
	...
	if [[ $2 != "" ]];then
		...
		if [[ $(echo $rom_str | tr A-Z a-z) == "lineageos" ]];then
			custom_branches=($(cat $custom_json | grep name | sed 's/"name"://g' | sed 's/"//g' |  sed 's/,//g' | sed 's/[[:space:]]//g' | grep "lineage") lineage-19.1 lineage-20.0 lineage-21.0 lineage-22.0)
		else
...
auto_build(){
	...
	wxpush_sucess_str="wxpusher_status ${pusher_script_success_str} 'Build Passed' >/dev/null 2>&1 && trap - SIGINT EXIT"
	wxpush_error_str="wxpusher_status ${pusher_script_error_str} "$(base64 < out/error.log)"  >/dev/null 2>&1 "

	trap "${wxpush_error_str}" EXIT
  	if [[ "$build_rom_cmd1" != "" ]];then
   		eval "$build_rom_cmd"
   		eval "$build_rom_cmd1" && eval ${wxpush_sucess_str}
     	else
      		eval "$build_rom_cmd" && eval ${wxpush_sucess_str}
     	fi
